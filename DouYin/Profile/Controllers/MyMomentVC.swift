...
extension MyMomentVC: UITableViewDelegate, UITableViewDataSource{
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if moment.type == 0 {
            ...
            if moment.pictures.count == 1 {
                let imageView = UIImageView(frame: CGRect(x: 0, y: 0, width: containerWidth, height: containerWidth))
                imageView.sd_setImage(with: URL(string: (moment.pictures[0])), placeholderImage: UIImage(named: "placeholder.png"))
                (cell as! ImageMomentCell).imageContainer.addSubview(imageView)
            }else if moment.pictures.count == 2 {
                for i in 0..<2 {
                    ...
                    imageView.sd_setImage(with: URL(string: (moment.pictures[i])), placeholderImage: UIImage(named: "placeholder.png"))
                    (cell as! ImageMomentCell).imageContainer.addSubview(imageView)
                }
            }else{
                for i in 0..<4 {
